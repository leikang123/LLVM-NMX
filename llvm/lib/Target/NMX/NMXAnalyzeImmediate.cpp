//===-- NMXAnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
NMXAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOperand(I) {}

// Add I to the instruction sequences.
void NMXAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void NMXAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void NMXAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void NMXAnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void NMXAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void NMXAnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
...
void NMXAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const NMXAnalyzeImmediate::InstSeq
&NMXAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
