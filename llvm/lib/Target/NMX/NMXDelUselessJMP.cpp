//===-- NMXDelUselessJMP.cpp - NMX Delete JMP Pass ------------*- C++ -*-===//
//
...
runOnMachineBasicBlock(MachineBasicBlock &MBB, MachineBasicBlock &MBBN) {
  ...
  if (I->getOpcode() == NMX::JMP && I->getOperand(0).getMBB() == &MBBN) {
...
/// createNMXDelJmpPass - Return a pass that DelJmp in NMX MachineFunctions
FunctionPass *llvm::createNMXDelJmpPass(NMXTargetMachine &tm) {
