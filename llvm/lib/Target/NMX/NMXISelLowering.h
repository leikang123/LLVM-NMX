//===-- NMXISelLowering.h - NMX DAG Lowering Interface --------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_NMX_NMXISELLOWERING_H
#define LLVM_LIB_TARGET_NMX_NMXISELLOWERING_H
...
enum NodeType {
  ...
  // No relation with NMX Hi register
  Hi,

  // Get the Lower 16 bits from a 32-bit immediate
  // No relation with NMX Lo register
  Lo,
...
class NMXTargetLowering : public TargetLowering {
public:
  explicit NMXTargetLowering(const NMXTargetMachine &TM,
                              const NMXSubtarget &STI);

  static const NMXTargetLowering *create(const NMXTargetMachine &TM,
                                          const NMXSubtarget &STI);
...
protected:
  ...
  SDValue getAddrLocal(NodeTy *N, EVT Ty, SelectionDAG &DAG) const {
    SDLoc DL(N);
    unsigned GOTFlag = NMXII::MO_GOT;
    SDValue GOT = DAG.getNode(NMXISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
                              getTargetNode(N, Ty, DAG, GOTFlag));
    SDValue Load =
        DAG.getLoad(Ty, DL, DAG.getEntryNode(), GOT,
                    MachinePointerInfo::getGOT(DAG.getMachineFunction()));
    unsigned LoFlag = NMXII::MO_ABS_LO;
    SDValue Lo = DAG.getNode(NMXISD::Lo, DL, Ty,
  ...
  SDValue getAddrGlobal(NodeTy *N, EVT Ty, SelectionDAG &DAG,
    ...
    SDLoc DL(N);
    SDValue Tgt = DAG.getNode(NMXISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
  ...
  SDValue getAddrGlobalLargeGOT(NodeTy *N, EVT Ty, SelectionDAG &DAG,
                                ...
                                const MachinePointerInfo &PtrInfo) const {
    SDLoc DL(N);
    SDValue Hi = DAG.getNode(NMXISD::Hi, DL, Ty,
                             getTargetNode(N, Ty, DAG, HiFlag));
    Hi = DAG.getNode(ISD::ADD, DL, Ty, Hi, getGlobalReg(DAG, Ty));
    SDValue Wrapper = DAG.getNode(NMXISD::Wrapper, DL, Ty, Hi,
                                  getTargetNode(N, Ty, DAG, LoFlag));
  ...
  SDValue getAddrNonPIC(NodeTy *N, EVT Ty, SelectionDAG &DAG) const {
    SDLoc DL(N);
    SDValue Hi = getTargetNode(N, Ty, DAG, NMXII::MO_ABS_HI);
    SDValue Lo = getTargetNode(N, Ty, DAG, NMXII::MO_ABS_LO);
    return DAG.getNode(ISD::ADD, DL, Ty,
                       DAG.getNode(NMXISD::Hi, DL, Ty, Hi),
                       DAG.getNode(NMXISD::Lo, DL, Ty, Lo));
  }
  ...
  public:
    ...
    NMXCC(CallingConv::ID CallConv, bool IsO32, CCState &Info,
...
private:
  ...
  NMXCC::SpecialCallingConvType getSpecialCallingConv(SDValue Callee) const;
  ...
  isEligibleForTailCallOptimization(const NMXCC &NMXCCInfo,
                                    ...
                                    const NMXMachineFunctionInfo &FI) const = 0;
...
createNMXSETargetLowering(const NMXTargetMachine &TM,
                           const NMXSubtarget &STI);
